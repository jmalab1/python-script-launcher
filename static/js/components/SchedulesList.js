...
import { schedules, profiles, workflows, tags, selectedScheduleTag, TRASH_GROUP } from '../state.js';
import { tagColor } from '../tagColors.js';
import { toggleSchedule, deleteSchedule, restoreSchedule, permanentDeleteSchedule, runScheduleNow, loadSchedules, duplicateSchedule } from '../api.js';
...
function getScheduleTags(schedule) {
    const targetItems = schedule.target_type === 'workflow' ? workflows.value : profiles.value;
    const target = targetItems.find(item => item.id === schedule.target_id);
    return (target?.tags || []).filter(id => tags.value.some(t => t.id === id));
}
...
export function SchedulesList({ onEdit }) {
    ...
    const selectedTag = selectedScheduleTag.value;

    const mainItems = [];
    const trashItems = [];
    for (const s of list) {
        if ((s.group || '') === TRASH_GROUP) trashItems.push(s);
        else mainItems.push(s);
    }

    let visible = mainItems;
    if (selectedTag) {
        visible = mainItems.filter(s => {
            const tagIds = getScheduleTags(s);
            if (selectedTag === 'untagged') return tagIds.length === 0;
    ...
    async function handleConfirmPermanentDelete() {
        ...
        await permanentDeleteSchedule(pendingPermanentDelete.id);
    ...
    function renderScheduleCard(s) {
        const isTrashed = (s.group || '') === TRASH_GROUP;
        const missing = !s.target_name;
        const targetTrashed = s.target_trashed;
        const blocked = missing || targetTrashed;
        ...
        const itemTags = (target?.tags || [])
            ...
            .filter(Boolean);
        return html`
            <div class="bg-white dark:bg-gray-800 shadow-xs rounded-xl p-4 border border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 transition flex flex-col gap-3">
                <div class="flex flex-wrap items-center gap-2">
                    ${isTrashed ? html`
                        <span class="w-2 h-2 rounded-full shrink-0 bg-gray-300 dark:bg-gray-600" title="Trashed"></span>
                    ` : html`
                    ...
                    `) : ''}
                    ${blocked && !isTrashed ? html`
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/20" title=${targetTrashed ? 'Target is in the trash' : 'Target no longer exists'}>
                            ${targetTrashed ? 'Target in trash' : 'Target missing'}
                        </span>
                    ` : ''}
                </div>
                ...
                <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
                    <span>
                        ...
                        ${isTrashed || blocked || !s.enabled ? html`<span class="text-gray-400 dark:text-gray-500">—</span>`
                ...
                <div class="flex items-center gap-1 flex-wrap justify-start border-t border-gray-100 dark:border-gray-700/40 pt-3">
                    ${isTrashed ? html`
                        ...
                        <button onClick=${() => setPendingPermanentDelete(s)} title="Permanently delete" aria-label="Permanently delete"
    ...
    function renderTrashSection() {
        ...
        return html`
            <div class="mb-4">
                <button onClick=${() => setTrashCollapsed(c => !c)}
                    ...
                    <svg class="w-3 h-3 transition-transform ${trashCollapsed ? '' : 'rotate-90'}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                ...
                <div class="${trashCollapsed ? 'hidden' : ''}">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        ${trashItems.map(s => renderScheduleCard(s))}
                    </div>
                </div>
            </div>
        `;
    ...
    return html`
        <${TagFilter}
            ...
            onSelect=${(v) => { selectedScheduleTag.value = v; }}
        />
        ${!visible.length && !trashItems.length ? html`
            <div class="text-center py-8 text-gray-500 dark:text-gray-400 text-sm">${selectedTag ? 'No schedules with this tag.' : 'No schedules yet.'}</div>
        ` : !visible.length ? html`
            <div class="text-center py-8 text-gray-500 dark:text-gray-400 text-sm">No schedules with this tag.</div>
        ` : html`
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                ${visible.map(s => renderScheduleCard(s))}
            </div>
        `}
        ${renderTrashSection()}
        <${ConfirmModal}
            ...
            message=${html`This will move <span class="font-medium text-gray-700 dark:text-gray-200">${pendingDelete ? (pendingDelete.name || pendingDelete.cron) : ''}</span> to the trash.`}
        />
        <${ConfirmModal}
            ...
            message=${html`This will permanently delete <span class="font-medium text-gray-700 dark:text-gray-200">${pendingPermanentDelete ? (pendingPermanentDelete.name || pendingPermanentDelete.cron) : ''}</span>. This action cannot be undone.`}
        />
    `;
}
