    ...
    return html`
        <aside class="hidden md:flex flex-col w-48 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700/60 shrink-0">
            <div class="flex items-center gap-2.5 px-5 py-4 border-b border-gray-200 dark:border-gray-700/60">
                ...
                <span class="text-[15px] font-bold text-gray-800 dark:text-gray-100 tracking-tight">Launch Control</span>
            </div>
...
export function MobileHeader() {
    ...
    return html`
        ...
        <header class="flex items-center gap-3 px-4 py-3 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700/60 md:hidden fixed top-0 left-0 right-0 z-30">
            <button onClick=${() => { sidebarOpen.value = !sidebarOpen.value; }}
                class="p-1.5 rounded-lg text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/50 shrink-0">
            ...
            <svg class="w-6 h-6 text-violet-500 shrink-0" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><path d="M31.956 14.8C31.372 6.92 25.08.628 17.2.044V5.76a9.04 9.04 0 0 0 9.04 9.04h5.716ZM14.8 26.24v5.716C6.92 31.372.63 25.08.044 17.2H5.76a9.04 9.04 0 0 1 9.04 9.04Zm11.44-9.04h5.716c-.584 7.88-6.876 14.172-14.756 14.756V26.24a9.04 9.04 0 0 1 9.04-9.04ZM.044 14.8C.63 6.92 6.92.628 14.8.044V5.76a9.04 9.04 0 01-9.04 9.04H.044Z" fill="currentColor"/></svg>
            <span class="text-sm font-bold text-gray-800 dark:text-gray-100 shrink-0">Launch Control</span>
            ...
            <div class="flex gap-1 overflow-x-auto shrink mob-nav-scroll">
                <button onClick=${() => showPanel('profiles')}
                    class="mob-nav px-2.5 py-1 rounded-md text-xs font-medium whitespace-nowrap ${currentPanel.value === 'profiles' ? 'text-violet-500 bg-violet-500/10' : 'text-gray-500 dark:text-gray-400'}">Profiles</button>
                <button onClick=${() => showPanel('workflows')}
                    class="mob-nav px-2.5 py-1 rounded-md text-xs font-medium whitespace-nowrap ${currentPanel.value === 'workflows' ? 'text-violet-500 bg-violet-500/10' : 'text-gray-500 dark:text-gray-400'}">Workflows</button>
                <button onClick=${() => showPanel('schedules')}
                    class="mob-nav px-2.5 py-1 rounded-md text-xs font-medium whitespace-nowrap ${currentPanel.value === 'schedules' ? 'text-violet-500 bg-violet-500/10' : 'text-gray-500 dark:text-gray-400'}">Schedules</button>
                <button onClick=${() => showPanel('audit')}
