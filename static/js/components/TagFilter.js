...
export function TagFilter({ tags, items, getTags, selectedTag, onSelect }) {
    ...
    function Pill(label, count, value, icon, color) {
        const isActive = selectedTag === value;
        // Tag pills carry their tag's color; "All" and "Untagged" stay gray
        // and use violet when selected, as before.
        const idle = color
            ? color.chip
        ...
        return html`
            <button onClick=${() => onSelect(isActive ? null : value)}
                class="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium transition whitespace-nowrap ${isActive ? active : idle}">
    ...
    return html`
        <div class="sticky top-0 z-10 flex flex-wrap gap-1.5 mb-3 bg-gray-50 dark:bg-gray-900 -mx-1 px-1 -my-1 py-1">
            ${Pill('All', items.length, null, null, null)}
            ${untaggedCount ? Pill('Untagged', untaggedCount, 'untagged', html`<svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6z"/></svg>`, null) : ''}
            ${activeTags.map(f => Pill(f.name, tagCounts[f.id], f.id, TAG_ICON, tagColor(f)))}
        </div>
    `;
}
